-- Unified College Application Database Schema (Safe Migration)
...
INSERT INTO public.unified_students (register_number, name, email, class_year) VALUES
    ('620123205001', 'Test Student 1', 'test1@college.edu', 'II-IT'),
    ('620123205002', 'Test Student 2', 'test2@college.edu', 'III-IT')
ON CONFLICT (register_number) DO NOTHING;

-- Insert sample registrations for test students
...
WHERE us.register_number IN ('620123205001', '620123205002')
ON CONFLICT (student_id, registration_type) DO NOTHING;

-- Optional: Try to migrate existing data if tables exist
DO $$
BEGIN
    -- Check if ii_it_students table exists and has data
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'ii_it_students') THEN
        -- Try to insert existing students (this will use whatever columns are available)
        INSERT INTO public.unified_students (register_number, name, class_year)
        SELECT 
            register_number,
            COALESCE(
                CASE WHEN EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'ii_it_students' AND column_name = 'name') 
                     THEN name 
    ...
    -- Check if iii_it_students table exists and has data
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'iii_it_students') THEN
        INSERT INTO public.unified_students (register_number, name, class_year)
        SELECT 
            register_number,
            COALESCE(
                CASE WHEN EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'iii_it_students' AND column_name = 'name') 
                     THEN name 
                     ELSE register_number 
                END, 
                register_number
    ...
    -- Register existing students for both services
    INSERT INTO public.unified_student_registrations (student_id, registration_type)
    SELECT 
        us.id,
        'both' as registration_type
    FROM public.unified_students us
    WHERE us.class_year IN ('II-IT', 'III-IT')
    ON CONFLICT (student_id, registration_type) DO NOTHING;

EXCEPTION WHEN others THEN
    -- If migration fails, just continue - the tables are created successfully
    NULL;
END $$;
