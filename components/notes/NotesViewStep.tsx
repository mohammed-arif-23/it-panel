...
interface NotesViewStepProps {
  data: NotesViewData
  onBack: () => void
}

export function NotesViewStep({ data, onBack }: NotesViewStepProps) {
  ...
  const handleDownload = async (file: CloudinaryFile) => {
    ...
    try {
      ...
      if (Capacitor.isNativePlatform()) {
        ...
        await notificationService.notifyNotesDownloaded(fileName)
        return
      }
  ...
  const fetchFiles = async (subjectCode: string) => {
    try {
      setLoading(true)
      setError('')
      const res = await fetch(`/api/notes/list?subject=${encodeURIComponent(subjectCode)}`, { cache: 'no-store' })
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="text-center">
          <div className="w-14 h-14 bg-gradient-to-br from-purple-100 to-violet-100 rounded-2xl flex items-center justify-center mx-auto mb-2 shadow-sm">
            <FileText className="w-7 h-7 text-purple-600" />
          </div>
          <h3 className="text-xl font-bold text-[var(--color-primary)]">Notes</h3>
      ...
      <div className="saas-card p-6">
        <div className="flex items-center justify-between mb-4">
          <h4 className="font-semibold text-[var(--color-primary)]">Available Notes</h4>
          <button
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <h5 className="text-lg font-medium text-[var(--color-text-secondary)] mb-2">No Notes Found</h5>
                ...
                <div key={file.public_id} className="p-4 bg-[var(--color-accent)] rounded-xl hover:shadow-md transition-all border border-[var(--color-border-light)]">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3 flex-1 min-w-0">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center space-x-3 text-xs text-[var(--color-text-muted)]">
                          ...
                          <span className="px-2 py-0.5 bg-purple-50 text-purple-700 rounded uppercase font-medium">
                            {file.format || 'file'}
                          </span>
                        </div>
                      </div>
                    ...
                    <Button
                      ...
                      size="sm"
                      className="flex items-center gap-2 bg-gradient-to-r from-purple-600 to-violet-600 hover:from-purple-700 hover:to-violet-700 text-white shadow"
                    >
                      {isDownloading ? (
