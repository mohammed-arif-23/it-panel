...
import type { NotesBrowseStepType } from './NotesBrowseFlow'
...
interface NotesBrowseStepProps {
  data: Partial<NotesBrowseData>
  onNext: (data: Partial<NotesBrowseData>) => void
  onBack: () => void
  currentStep: NotesBrowseStepType
}

export function NotesBrowseStep({ data, onNext, onBack, currentStep }: NotesBrowseStepProps) {
    ...
    try {
      ...
      if (currentStep === 'department') {
        ...
        if (!data.department || !data.year || !data.semester) {
          console.log('Missing data for notes subjects fetch:', { department: data.department, year: data.year, semester: data.semester })
          setError('Missing required data for fetching subjects')
          return
        }
        console.log('Fetching notes subjects for:', { department: data.department, year: data.year, semester: data.semester })
        const res = await fetch(`/api/notes/subjects?dept=${encodeURIComponent(data.department)}&year=${encodeURIComponent(data.year)}&semester=${encodeURIComponent(data.semester)}`, { cache: 'no-store' })
        const json = await res.json()
        console.log('Notes subjects API response:', json)
    ...
    <div className="space-y-6">
      ...
      <div className="text-center">
        ...
        <p className="text-gray-600">
          ...
          {currentStep === 'subject' && `Theory subject for ${data.department || ''} - ${data.year || ''}${data.semester ? ` - ${data.semester}` : ''}`}
        </p>
      </div>
