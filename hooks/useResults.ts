  ...
  return useQuery<ResultsResponse>({
    ...
    queryFn: async () => {
      ...
      const response = await fetch(
        `/api/results?register_number=${encodeURIComponent(registerNumber)}`
      );
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabaseAdmin
        .from("subjects")
        .select(
          "code, credits, is_skill_based, is_ncc_course, is_non_credit, course_type, category"
        );
...
// Note: GPA calculation now uses sum of individual credits to match college method
...
export function calculateGPA(
  ...
  if (hasU) {
    ...
    // UNIVERSITY RULE: Use sum of individual credits (matches college calculation method)
    ...
    // Always use sum of individual credits (college method)
    const denominatorCredits = subjects.reduce((sum, [subjectCode]) => {
      const credits = subjectCredits?.[subjectCode];
      return sum + (typeof credits === "number" ? credits : 0);
    }, 0);

    const gpa =
      denominatorCredits > 0 ? totalWeightedPoints / denominatorCredits : 0;
    const roundedGpa = Math.round(gpa * 100) / 100;
...
export function getSemesterStatus(gpa: number): {
  status: string;
  color: string;
} {
