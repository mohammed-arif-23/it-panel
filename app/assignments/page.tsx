...
export default function AssignmentsPage() {
  ...
  const { data: assignments = [], isLoading } = useAssignments(user?.id || '', user?.class_year || '')
  ...
  const handleFileSelect = useCallback((e: React.ChangeEvent<HTMLInputElement>, assignmentId: string) => {
  ...
  const handleSubmission = useCallback(async (assignmentId: string) => {
    ...
    try {
      ...
      // Step 2: Submit to database with Cloudinary URL using mutation
      ...
      setUploadMessage(`🎉 Assignment submitted successfully! You got ${result.marks} marks.`)
  ...
  }, [selectedFile, user, submitAssignmentMutation])
...
const AssignmentCard = memo(({ 
  ...
  onFileSelect: (e: React.ChangeEvent<HTMLInputElement>, assignmentId: string) => void
  onSubmit: (assignmentId: string) => void
  formatDate: (dateString: string) => string
  isOverdue: (dueDate: string) => boolean
}) => (
  <Card className="group relative overflow-hidden bg-white shadow-2xl border-0 hover:shadow-3xl transition-all duration-500 hover:-translate-y-1">
                ...
                <div className="space-y-4">
                  <div className="p-4 bg-gradient-to-r from-blue-50 to-cyan-50 rounded-xl border border-blue-200">
                    ...
                    <input
                      ...
                      onChange={(e) => onFileSelect(e, assignment.id)}
