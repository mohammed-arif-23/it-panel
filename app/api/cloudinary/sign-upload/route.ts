...
export async function POST(request: NextRequest) {
  try {
    ...
    // Parse request body for an optional folder parameter
    const body = await request.json().catch(() => ({})) as { folder?: string }
    const requestedFolder = typeof body?.folder === 'string' ? body.folder.trim() : ''

    // Whitelist: allow default 'assignments' OR 'questionpapers/<subject_code>'
    // Auto-create in Cloudinary via upload if it doesn't exist
    ...
    // Only include parameters that should be part of the signature
    // Cloudinary signed uploads require: timestamp, folder (optional)
    const paramsToSign = {
